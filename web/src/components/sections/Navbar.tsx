  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <motion.div
        ...
        className={`relative z-50 transition-[background,box-shadow] duration-300 ${!open && scrolled ? "nav-glass shadow-[0_10px_40px_-20px_rgba(0,0,0,0.8)]" : ""}`}
      >
        <nav className="container-x flex h-[68px] items-center justify-between gap-4" aria-label="Asosiy navigatsiya">
          ...
          <ul className="hidden items-center gap-1 lg:flex">
            {NAV_ITEMS.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="group relative rounded-full px-3.5 py-2 text-[0.92rem] font-semibold text-white/65 transition-colors hover:text-white">
                  {item.label}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-navy-950 text-white lg:hidden"
          >
            <div className="pointer-events-none absolute inset-0" aria-hidden="true">
              ...
              <Footprint className="absolute right-[-8%] top-[14%] w-[52%] rotate-12 fill-white/[0.04]" />
            </div>
            <div className="container-x relative flex h-full flex-col pt-[88px] pb-8">
                ...
                {NAV_ITEMS.map((item, i) => (
                  ...
                  >
                    <Link
                      ...
                      className="flex items-center justify-between rounded-2xl px-3 py-4 font-display text-[1.7rem] font-semibold tracking-tight hover:bg-white/5"
                    >
                      {item.label}
                      <Icon name="arrow" className="size-6 text-green" />
                    </Link>
                  </motion.li>
                ))}
              </ul>
              ...
              >
                ...
                <div className="flex gap-3">
                  {CONTACT.phone && (
                    <a href={telHref(CONTACT.phone)} className="inline-flex h-12 flex-1 items-center justify-center gap-2 rounded-full border border-white/15 font-bold">
